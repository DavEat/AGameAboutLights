...
public class AG_LightPrisma : AG_LightCaster
{
    ...
    public override void Cast(int _currentLightHeadIndex, int _colorIndex, Vector2 _origin, Vector2 _hitPoint, Vector2 _normal, float _previousAnimDuration, int _lightIndex)
    {
        ...
        AG_LightsManagementNew.inst.AddLightHead(AG_LightsManagementNew.inst.firstListLightHead, InitLightHead(((AG_PrismaFace)_elem).face1, _hitPoint, colorsIndexs[0], _previousAnimDuration));
        AG_LightsManagementNew.inst.AddLightHead(AG_LightsManagementNew.inst.firstListLightHead, InitLightHead(((AG_PrismaFace)_elem).face2, _hitPoint, colorsIndexs[1], _previousAnimDuration));
    ...
    private LightHead InitLightHead(Transform _face, Vector2 _hitPoint, int _colorIndex, float _previousAnimDuration)
    {
        ...
        return new LightHead(_colorIndex, origin, direction, _face, _previousAnimDuration);
    }
